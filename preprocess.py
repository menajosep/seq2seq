...
class processor(processor_base):
    ...
    def fit(self,
        ...
        logging.warning('(1/2) done. %s sec' % str(time_diff(now)))
        logging.warning('....building corpus')
        now = get_time()
        self.indexer = custom_Indexer(num_words=self.keep_n,bad_words_list=self.bad_words_list)
        ...
        # logging
        logging.warning('(2/2) done. %s sec' % str(time_diff(now)))
    ...
    def fit_transform(self,
        ...
        logging.warning('done. %s sec' % str(time_diff(now)))
        return final_data
...
class custom_Indexer(Tokenizer):
    ...
    def __init__(self, num_words, bad_words_list):
    ...
    def fit_on_tokenized_texts(self, tokenized_texts):
            ...
            word_index = dict(list(zip(sorted_voc, list(range(2, len(sorted_voc) + 2)))))
            if self.bad_words_list:
                for w in self.bad_words_list:
                    if w in word_index:
                        word_index[w] = 1
